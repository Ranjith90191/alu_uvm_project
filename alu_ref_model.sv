...
class alu_ref_model extends uvm_component;
  ...
  ord_stage_t ord_pipe[2];   // [0]=just committed, [1]=1 cycle old -> reveals next
      ...
      case (state)
        ...
        WAIT_B: begin
          if (pkt.inp_valid == 2'b11) begin
            ...
            oprd1=pkt.OA; p_cmd=pkt.cmd; p_mode=pkt.mode;   // latest-A-wins
            wait_counter=0;
            ...
            // inp_valid==00 lands here now -- HOLD, not clear.
            // Reinterpretation of spec's "00=clear" for the WAIT states
            // specifically -- see message text. Needed to make the
            // 16-cycle timeout reachable at all.
            wait_counter++;
          end
        end
        ...
        WAIT_A: begin
          if (pkt.inp_valid == 2'b11) begin
            ...
            oprd2=pkt.OB; p_cmd=pkt.cmd; p_mode=pkt.mode;   // latest-B-wins
            wait_counter=0;
          end else if (pkt.cmd != p_cmd || pkt.mode != p_mode) begin
            p_cmd=pkt.cmd; p_mode=pkt.mode; wait_counter=0;
          end else if (wait_counter==15) begin
            ...
            wait_counter++;   // same reinterpretation as WAIT_B
          end
        end
        ...
        MUL_BUSY: begin
          if (mul_gap_pending) begin
            ...
            if (pkt.inp_valid==2'b11 && pkt.mode==1'b1 &&
              ...
              // ASSUMPTION: idle during the gap changes nothing
            end
            else begin
              abort_mul();
              try_capture(pkt);
  ...
  function void try_capture(alu_seq_item pkt);
    if (pkt.inp_valid == 2'b11) begin
      ...
      state = IDLE;   // 00 from IDLE: nothing captured, nothing to hold or clear
    end
  endfunction
  ...
  function void timeout();
    ...
    nv.err=1'b1;   // ASSUMPTION: registered, same latency as everything else
  ...
  function void compute(logic [`DW-1:0] a, b, logic [`CW-1:0] cmd, logic mode, logic cin);
    ...
    if (mode) begin // ---------------- arithmetic ----------------
      case (cmd)
        ...
        4'b0001: begin // SUB -- full sign-extend to 2*DW, not zero-extend
          diff = a - b;
        ...
        4'b0011: begin // SUB_CIN -- same sign-extend fix
          diff = a - b - cin;
          nv.res = {{`DW{diff[`DW-1]}}, diff};
          nv.oflow = (a[`DW-1]!=b[`DW-1]) && (diff[`DW-1]!=a[`DW-1]);
        end
